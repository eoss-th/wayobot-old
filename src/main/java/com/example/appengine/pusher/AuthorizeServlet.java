 ...
 * for private, presence channels. Successful authentication returns valid
 * authorization token with user information.
 *
 * @see <a href="https://pusher.com/docs/authenticating_users">Pusher
 *      Authentication Docs</a>
 */
...
public class AuthorizeServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		String accountId = request.getRequestURI().replace("/authorize/", "");
		// Instantiate a pusher connection
		Pusher pusher = PusherService.getPusher(accountId);
		// Get current logged in user credentials
		String user = (String) request.getSession().getAttribute("token");
		// redirect to homepage if user is not authorized
		if (user == null) {
			user = UUID.randomUUID().toString();
		}
		
		String displayName = (String) request.getSession().getAttribute("subAccName");

		String query = CharStreams.toString(request.getReader());
		// socket_id, channel_name parameters are automatically set in the POST body of
		// the request
		...
		// Inject custom authentication code for your application here to allow/deny
		// current request

		String auth = pusher.authenticate(socketId, channelId, new PresenceUser(request.toString(), userInfo));
